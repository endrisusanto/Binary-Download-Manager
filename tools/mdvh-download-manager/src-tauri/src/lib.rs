...
use mdvh_agent_probe::{run_probe, parse_workflow_file, ProbeOptions, DownloadProgress, ProgressCallback};
...
async fn start_payload_listener(app_handle: AppHandle, port: u16) {
    ...
    let listener = match TcpListener::bind(&bind_addr).await {
        Ok(l) => {
            let _ = app_handle.emit("listener-status", serde_json::json!({
                "active": true,
        ...
        if let Ok((mut stream, _)) = listener.accept().await {
            ...
            tokio::spawn(async move {
                if let Err(e) = handle_stream(&app, &mut stream).await {
                    let _ = app.emit("download-error", format!("Error reading payload connection: {}", e));
                }
            });
        }
    }
...
async fn handle_stream(app: &AppHandle, stream: &mut tokio::net::TcpStream) -> anyhow::Result<()> {
    ...
    loop {
        ...
        if read_total >= 4 && buffer[..read_total].windows(4).any(|w| w == b"\r\n\r\n") {
            ...
            let body = &buffer[header_end..header_end + content_length.min(read_total - header_end)];
            ...
            let response_body = serde_json::json!({ "ok": true, "path": payload_path.to_string_lossy() }).to_string();
            let response = format!(
...
async fn trigger_download(app: &AppHandle, payload_path: std::path::PathBuf) -> anyhow::Result<()> {
    ...
    tokio::spawn(async move {
        ...
        let _ = app_clone.emit("download-started", serde_json::json!({
